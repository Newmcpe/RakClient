//! Cipher tables extracted byte-exact from RakSAMP Lite.exe (samp.dll's port cipher).
//!
//! `SUBSTITUTION` is `byte_499C60` @0x499C60 (the 256-byte substitution table used by
//! `sub_419060`). `MASK_BYTE` is the uniform value of `xmmword_4A11E0` @0x4A11E0 (16 bytes,
//! every lane `0xAA`), so the SIMD lane-masked checksum and the scalar tail both reduce to
//! `XOR over data bytes of (byte & 0xAA)`.
//!
//! Generated from `get_bytes` output; do not edit by hand.

pub(crate) const SUBSTITUTION: [u8; 256] = [
...
/// Checksum mask: `xmmword_4A11E0` is 16 lanes of this value.
pub(crate) const MASK_BYTE: u8 = 0xAA;

/// Inverse of [`SUBSTITUTION`], built at compile time so the table has a single source of truth.
pub(crate) const INVERSE: [u8; 256] = {
    let mut inv = [0u8; 256];
    let mut i = 0;
    while i < 256 {
        inv[SUBSTITUTION[i] as usize] = i as u8;
        i += 1;
    }
    inv
};
