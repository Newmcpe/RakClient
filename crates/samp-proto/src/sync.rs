...
//! bit-packed sync (`PlayerSyncData`/`VehicleSyncData`) and raw-struct sync (aim/bullet/trailer/…).
//! See docs/memory/samp-proto/sync.md#module
...
/// Sync packets are sender-only, so `Packet`+`Encode` is identical boilerplate; `decode`/`write` stay hand-written.
...
/// Inverse of [`decompress_health_and_armor`]; only multiples of 7 (and `>= 100`) survive a decompress→compress round-trip.
...
/// In-vehicle driver sync (`ID_VEHICLE_SYNC` = 200); `train_speed`/`trailer_id` are bool-flagged optionals.
/// Field names are binary-verified (`SAMP_Packet_VehicleSync` @0x1000A6B0); see docs/memory/samp-proto/sync.md#vehicle-sync
#[derive(Debug, Clone, Copy, PartialEq, Default)]
