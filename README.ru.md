...
RakClient — это headless-клиент SA-MP 0.3.7 (Rust + async Tokio): автономный бот, под который вы пишете
игровые сценарии. Ни GTA:SA, ни движка, ни графики. Настоящий SA-MP — это GTA:SA плюс `samp.dll`;
RakClient игру не запускает и сам говорит с сервером по сети, как это делал бы настоящий клиент.
Протокол восстановлен из оригинального `RakSAMP Lite.exe`, поэтому это полноценный клиент: он
подключается к живым серверам (в том числе Arizona RP), проходит весь путь от подключения до спавна, а
дальше делает то, что вы ему напишете — работы, автоматизация, чат-мост или любой другой сценарий на
Lua.
...
![Лесопилка, навмеш с земли](docs/img/sawmill-navmesh-ground.png)
...
синхронизации на ногах и побайтовый шифр с таблицей замен на 256 байт. Скорости и анимации походок
сняты с живого 207-каптура. Эти значения не угадываются, они выверены по бинарю, поэтому менять их
можно только вместе с golden-vector тестом.
